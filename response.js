...
const outsideMenus = ['마라', '육쌈', '팬피그', '초원', '미세노센세', '토미야'];
...
function postText(
  ...
  text,
  username = 'Sikdanbot',
  icon_url = 'https://steamcdn-a.akamaihd.net/steamcommunity/public/images/avatars/59/5955c11d066b3f95c57e82a5388b2183cdfaa13e_full.jpg'
) {
  web.chat
    .postMessage({
      channel: channel,
...
function postAttachments(channel, attachments) {
  web.chat
    .postMessage({
      ...
      username: 'Sikdanbot',
      icon_url:
        'https://pbs.twimg.com/profile_images/975638064348327936/26mLY9Qf_400x400.jpg',
...
module.exports = {
  ...
  randomOutside: function(message) {
    postText(
      message.channel,
      `하와와.... 랜덤봇쟝의 추천은 ${
        outsideMenus[gen.create()(outsideMenus.length)]
      }인 거시야요....`,
      '랜덤봇쟝'
    );
  },
        ...
        postAttachments(message.channel, [
          {
            ...
            title: `${getToday()}의 식단`,
            title_link:
              'https://bds.bablabs.com/restaurants?campus_id=JEnfpqCUuR',
            text: menu,
  ...
  randomChoice: function(message) {
    ...
    if (textTokens[0] === '!랜덤') {
      ...
      postText(
        message.channel,
        `하와와.... 랜덤봇쟝의 추천은 ${
          choices[gen.create()(choices.length)]
        }인 거시에요....`,
        '랜덤봇쟝',
